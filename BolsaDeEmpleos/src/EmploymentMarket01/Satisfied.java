...
public CollectionPersonApplicant getPendingPersonApplication() {
//setPendingPersonApplication();
return PendingPersonApplication;
}
public void setPendingPersonApplication() {
for(int i=0; i<CollectionPersonApplicant.getInstanceCollectionPersonApplicant().getPersonApplicantions().size(); i++){
if(CollectionPersonApplicant.getInstanceCollectionPersonApplicant().getPersonApplication(i).isSatisfied()==false)
PendingPersonApplication.setPersonApplicantions(CollectionPersonApplicant.getInstanceCollectionPersonApplicant().getPersonApplication(i));
}
}
public CollectionCompanyApplication getPendingCompanyApplication() {
//setPendingCompanyApplication();
return PendingCompanyApplication;
}
public void setPendingCompanyApplication() {
for(int i=0; i<CollectionCompanyApplication.getInstanceCollectionCompanyApplication().getCompanyApplications().size(); i++)
if(CollectionCompanyApplication.getInstanceCollectionCompanyApplication().getCompanyApplication(i).isSatisfied()==false)
PendingCompanyApplication.setCompanyApplications(CollectionCompanyApplication.getInstanceCollectionCompanyApplication().getCompanyApplication(i));
}
